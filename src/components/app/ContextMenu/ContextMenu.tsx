...
import { BarChart3, Calculator, ClipboardPaste, Copy, Edit3, Palette, Sparkles, Trash2, X, Scissors } from "lucide-react";
...
const ContextMenu: React.FC<ContextMenuProps> = ({ position, onClose, onOpenAIWithRange, isCell, selectedCells, cellId }) => {
  ...
  return (
    <div
      ref={menuRef}
      className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-lg py-2 min-w-[180px]"
      style={menuStyle}
    >
      {/* Header */}
      {/*<div className="flex items-center justify-between px-4 py-2 border-gray-200 dark:border-gray-700">*/}
      ...
      {/* Standard Actions */}
      <div className="px-2 py-1 border-gray-200 dark:border-gray-700">
        {/*<div className="text-xs font-medium text-gray-500 dark:text-gray-400 px-2 py-1">*/}
        {/*  Actions*/}
        {/*</div>*/}
        ...
        <button
          onClick={handleEdit}
          className="w-full flex items-center space-x-2 px-2 py-2 text-sm text-gray-800 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-md transition-colors"
        >
          <Scissors size={16} />
          <span>Cut</span>
        </button>
