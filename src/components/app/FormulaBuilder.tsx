...
const FormulaBuilder: React.FC<FormulaBuilderProps> = ({ onWidthChange }) => {
  ...
  // test functions
  const handleFunctionSelect = (func: FormulaFunction) => {
    setSelectedFunction(func);
    setFormula(func.syntax);
    targetSelectorRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    const panel = targetSelectorRef.current?.closest('.overflow-y-auto');
  ...
  const handleTargetCellChange = (e: React.ChangeEvent<HTMLInputElement>) => {
