...
export default function useCell({ value, formula, col, row, style}: CellProps) {
  ...
  // Save changes
  const saveCellChange = () => {
    if (draftValue !== value || draftFormula !== formula) {
      // Here you would typically call a function to update the cell in your data store
      console.log(`Saving cell ${cellId}:`, { value: draftValue, formula: draftFormula });
    }
    setIsEditing(false);
    setIsActive(false);
  }
