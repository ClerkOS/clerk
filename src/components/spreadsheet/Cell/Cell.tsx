...
const Cell:  React.FC<CellProps> = ({
  ...
  const cellData = {value: value, type, formatted: value, formula: null, style: null}; // Placeholder for actual cell data
  ...
  // Handle key press
  // const handleKeyPress = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {    if (e.key === 'Enter') {
  //     e.preventDefault();
  //     handleCellUpdate();
    ...
    >
      {isEditing ? (
        <input
          ...
          className="w-full h-full bg-transparent border-none outline-none p-0 text-gray-800 dark:text-gray-100"
          // style={getCellStyles()}
        />
      ) : (
